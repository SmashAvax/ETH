 ...
 */
async function init({ rpc, noteNetId, currency = 'eth', amount = '0.1' }) {
  ...
  if (!(netId === 1 || netId === 42)) {
    throw new Error('You did not connect to SmartChain network. Please make sure you connected to SmartChain Mainnet')
  }
  ...
  if (smashnado._address === null) throw new Error('You are in other Network. Please make sure you connected to SmartChain Mainnet.')
  ...
  window.deposit = async (_currencyStr, _amountStr) => {
    ...
    try {
      ...
      if (noteString) {
        ...
        //var resultStr = 'SmashCash ETH balance is ' + web3.utils.fromWei(await web3.eth.getBalance(smashnado._address)) +'<br/>'
